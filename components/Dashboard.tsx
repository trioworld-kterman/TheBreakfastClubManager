...
export const Dashboard: React.FC<DashboardProps> = ({ data, onUpdate, onLogout }) => {
  ...
  const copyInviteLink = () => {
    const encoded = StorageService.encodeState(data);
    const shareUrl = `${window.location.origin}${window.location.pathname}?data=${encoded}`;
    
    navigator.clipboard.writeText(shareUrl);
    alert("Smart Link Copied! This link contains all current names and order. Send it to your team!");
  };
                ...
                {data.employees.length === 0 ? (
                  ...
                  data.employees.map((emp, idx) => (
                    <div 
                      ...
                      className={`group flex items-center justify-between p-5 bg-white rounded-2xl border-2 transition-all cursor-grab active:cursor-grabbing ${
                        draggedIdx === idx ? 'opacity-20 border-amber-900 border-dashed scale-95' : 'border-amber-50 hover:border-amber-950 shadow-sm hover:shadow-md'
                      }`}
                    >
                      <div className="flex items-center gap-4">
